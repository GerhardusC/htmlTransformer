...
use rocketseed_interview::{change_tag_content_case, TargetCase};
...
async fn transform_case_handler(Json(payload): Json<ReqBody>) -> (StatusCode, String) {
    ...
    let target_case = match transform.as_ref() {
        ...
        _ => {
            return (StatusCode::BAD_REQUEST, "Invalid value received for target case.".to_owned());
        }
    };
    match change_tag_content_case(&html, "p", target_case) {
        ...
        Err(_) => {
            return (StatusCode::BAD_REQUEST, "Failed to parse html content".to_owned());
        },
    }
}
